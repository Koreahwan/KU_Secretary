...
Google Calendar 연동은 별도 MCP 서버 없이 Google Calendar REST API로 동작합니다. OAuth 토큰 파일을 준비한 뒤 `.env` 또는 `config.toml`에 아래 값을 켭니다.

```bash
GOOGLE_CALENDAR_SYNC_ENABLED=true
GOOGLE_CALENDAR_ID=primary
GOOGLE_CALENDAR_TOKEN_FILE=data/google_calendar_token.json
GOOGLE_CALENDAR_CREDENTIALS_FILE=credentials/google_oauth_client.json
```

토큰 파일은 `access_token`을 직접 넣거나, 장기 사용을 위해 `refresh_token`과 OAuth 클라이언트 정보를 포함할 수 있습니다. `credentials/google_oauth_client.json`은 Google Cloud에서 받은 installed/web client JSON 형식을 그대로 사용할 수 있습니다.

연동 확인:

```bash
./.venv/bin/kus sync-google-calendar
```

`kus sync --all`과 `kus sync-uclass`는 `GOOGLE_CALENDAR_SYNC_ENABLED=true`일 때 UClass 과제/HW 마감과 시험·퀴즈성 일정을 Google Calendar에 upsert합니다. 같은 원본 항목은 deterministic event id로 갱신되므로 반복 동기화해도 중복 생성되지 않습니다.

캘린더 저장 규칙은 고정입니다.
- 제목: `강의이름 + 시험/발표/과제/퀴즈 중 하나`
- 제목의 강의이름에는 `00분반`, `05분반` 같은 분반 표기를 넣지 않음
- 일정 형태: Google Calendar 하루종일 이벤트로 저장
- 마감 시간이 `23:59`이면 제목에 시간을 표시하지 않고, `23:59`가 아니면 제목 뒤에 `HH:MM` 표시
- 분류는 LMS 원본 종류와 제목의 구체 키워드가 우선이며, 단순히 제목에 `시험/중간/기말`이 들어간다는 이유만으로 `시험`으로 바꾸지 않음
- `퀴즈` 원본은 `퀴즈`, 과제/제출/HW/프로젝트/리포트 원본은 `과제`, 발표 원본은 `발표`, 실제 시험 원본은 `시험`으로 저장
- 설명: 간단한 내용만 저장
- 알림: 모든 Google Calendar 알림 제거, 기본 알림도 사용하지 않음
- 완료/이미 지난 항목은 제목 뒤에 `[완료]` 표시
- 색상: 미완료 시험/과제/퀴즈/발표 일정은 파란색, 완료/지난 시험·과제·퀴즈·발표 일정은 회색으로 표시

캘린더 동기화 안전 규칙은 더 중요합니다.
- 사용자가 직접 추가한 Google Calendar 일정은 절대 수정하거나 삭제하지 않음
- KU Secretary가 만든 일정이라도 사용자가 Google Calendar에서 직접 날짜, 시간, 종일 여부 등 일정 정보를 바꾼 경우 절대 덮어쓰지 않음
- 단, 완료/미완료 표시만은 별도로 동기화할 수 있음. 사용자가 직접 만들었거나 일정 정보를 수정한 이벤트라도 제목/내용이 시험·과제·퀴즈·발표 또는 `기해실`/`Cykor`로 분류되면 색상을 적용함
- 위 분류 대상이 실제 Google Calendar 종료 시각 기준으로 지난 일정이면, 사용자 수정 여부와 무관하게 회색과 제목의 `[완료]` 표시를 적용함. 이 예외도 날짜, 시간, 종일 여부, 위치 등 일정 내용은 바꾸지 않음
- 색상 동기화는 지난 일정도 잡기 위해 기본 동기화 창보다 넓게 최근 1년 범위를 훑음
- 위 조건에 걸린 일정은 동기화 실패로 처리하지 않고 `skipped_events`에 기록한 뒤 건너뜀
- 자동 동기화가 수정할 수 있는 대상은 KU Secretary가 만들었고, 마지막 KU Secretary 동기화 이후 사용자가 일정 정보를 바꾸지 않은 이벤트뿐임
